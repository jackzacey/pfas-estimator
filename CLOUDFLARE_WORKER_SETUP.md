...
- `counter`: atomic reads or increments for the public ZIP-search counter.
...
## Configure the search counter

CounterAPI V1 has been removed. The ZIP-search counter now uses a Cloudflare D1 database through the same Worker.
...
No SQL setup is required. The Worker creates the counter table and record when it is first used. The badge loads the current total when the homepage opens, and valid ZIP searches increment it atomically.
...
Deploy the updated Worker prompt first, verify one chat and the ZIP-search counter, then push the website. The current production Worker already understands the chat request shape, so the deterministic lookup remains safe if the two deployments are separated; deploying the Worker first ensures the stricter system-association language is active before the new interface appears.
